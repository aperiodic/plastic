...
(defn add-transition
  ...
  Adding a dispatched transition does prevent plastic from checking a few
  ...
  transition into it, nor that the graph is connected. The only thing plastic
  ...
  plastic's init fn; otherwise you should stick to the DOM triggers."
    ...
    (into {} (for [state all-states]
               (let [outs (transitions-from sm state)
                     ...
                     with-custom (if (empty? custom-outs)
                                    {}
                                    {:plastic/event-recognizers custom-outs})]
...
(defn valid?
  ...
  machine in a plastic app. Specifically, that means:
