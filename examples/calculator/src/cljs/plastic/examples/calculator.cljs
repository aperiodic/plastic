(ns plastic.examples.calculator
  (:require [cljs.core.async :as async]
            [plastic.core :as plastic]
            [plastic.state-machine :as sm]
            [plastic.examples.calculator.core :as calc :refer [calculator new-state]]
            [om.core :as om]
            [om.dom :as dom]))
...
(defn start!
  ...
  (let [custom-event-channels (add-calculator-event-emitters!)]
    (plastic/init! (root-node) custom-event-channels calculator !state
                   {:logging true}))
  (om/root
    (fn [state _owner]
      (reify
